...
MResource* MSaveFileImpl::getNew()
{
    return new MSaveFileImpl();
}
...
void MSaveFileImpl::save(const char* filename, MSaveFileMode mode)
{
    m_filename = filename;
    m_mode = mode;
    save();
...
void MSaveFileImpl::load(const char* filename, MSaveFileMode mode)
{
    m_filename = filename;
    m_mode = mode;
    load();
...
{
    TiXmlDocument file(m_filename.c_str());
